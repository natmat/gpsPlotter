...
for line in log_file:
    ...
    if match:
        ...
        lat = gps_d["lat"]
        lon=  gps_d["lon"]
        confidence = gps_d["confidence"]

        if confidence.lower() == 'gps':
            icon_colour = 'green'
        folium.Marker(location=[lat, lon],
                      popup = gps_d,
                      icon=folium.Icon(color=random.choice(icon_colours), icon='cloud')).\
            add_to(gps_map)


        folium.Circle([lat, lon], radius=geofence, fill=True,
                      fill_color=random.choice(circle_colours), fill_opacity=0.25, tooltip=confidence).\
            add_to(gps_map)
